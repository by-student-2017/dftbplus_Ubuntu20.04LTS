...
if [ -d ./dftbplus-18.2 ];
then
  echo "detect old dftbplus-18.2 directory !"
  echo "delet old dftbplus-18.2 directory"
  sudo rm -f -r ./dftbplus-18.2
fi
#
...
make -j${num_core}
make test TEST_MPI_PROCS=${num_core} TEST_OMP_THREADS=1 
make install
